...
export const QueryResultsGridView = () => {
    ...
    useEffect(() => {
        async function restoreMaximizedGrid() {
            ...
            if (result?.resultId) {
                setMaximizedGridKey(result.resultId);
            }
        }
    ...
    const getGridHeight = () => {
        ...
        if (totalGrids === 0) return "300px";

        // If a grid is maximized, it takes 100% height
        if (maximizedGridKey !== undefined) return "100%";

        // Otherwise, divide height equally with a minimum of 300px
        const percentage = 100 / totalGrids;
        return `max(300px, ${percentage}%)`;
    };
    ...
    return (
        <div
            ...
            {gridList.map((item, index) => {
                ...
                return (
                    <div
                        key={item.batchId + "_" + item.resultId}
                        id={item.batchId + "_" + item.resultId}
